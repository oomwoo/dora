...
const short USER_CMD_MAX_AGE = 10; // RPi command expires after x20ms
...
short base_pwr_table[] = {50, 60, 70, 80, 90, 100, 110}; // Set by jumper
		...
		while (true)
		{
			...
			switch (uartCmd)
			{
				case 'c':
					...
					user_cmd_age = USER_CMD_MAX_AGE;
					break;
			}
		}

		// When Raspberry Pi runs slow, its last command
		// can expire
		if (user_cmd_age == 0)
			UserCmdFromRpi = USER_CMD_NONE;
		else
